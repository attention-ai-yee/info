...
const link = `${baseUrl}/alert?id=${encodeURIComponent(alertId)}&time=${encodeURIComponent(loginTime)}&location=${encodeURIComponent(loginLocation)}`;
...
<div style="font-family: system-ui, -apple-system, 'PingFang SC', 'Microsoft YaHei', sans-serif; max-width:600px; margin:0 auto; padding:32px 24px; color:#1f2330;">
  <div style="background:#fff; border-radius:12px; padding:32px; box-shadow:0 4px 16px rgba(0,0,0,.06);">
    ...
    <table cellpadding="0" cellspacing="0" border="0" style="margin:20px auto;">
      ...
      <tr><td align="center">
        <a href="${link}&action=report" style="display:block; padding:12px 0; width:220px; text-align:center; background:#e0413e; color:#fff; text-decoration:none; border-radius:6px; font-weight:600; font-size:15px;">非本人，立即处理</a>
      </td></tr>
    </table>
  </div>
